...
func get_item_by_name( val : String ) -> Dictionary:
	for item in items:
		if item["title"] == val:
			return item
...
func get_random_item(category : String = "") -> Dictionary:
...
func get_random_items(min_count: int, max_count: int, category : String = "") -> Array:
	var result = []
	var count = randi_range(min_count, max_count)  # Random number between min and max
